...
class HypothesisIsolator:
    """Combine residual, trend, timing, and dropout evidence via softmax scores.
    ...
    def infer(self, sensor: str, diagnostics: Mapping[str, float]) -> FaultDecision:
        ...
        evidence = {key: value / normalizer for key, value in exponentials.items()}
        ranked = sorted(evidence.items(), key=lambda item: item[1], reverse=True)
        fault, confidence = ranked[0]

        actions = {
            "nominal": "continue",
            "noisy": "inflate_covariance",
            "biased": "suppress_and_reestimate_bias",
            "delayed": "freeze_measurements_and_recalibrate_time",
            "miscalibrated": "freeze_and_refine_extrinsics",
            "failed": "isolate_sensor",
        }
        return FaultDecision(
            suspected_sensor=sensor,
            suspected_fault_type=fault,
            confidence=confidence,
            supporting_diagnostics=dict(diagnostics),
            competing_hypotheses=dict(ranked[1:]),
            recommended_action=actions[fault],
        )
