...
class RecoveryController:
    ...
    def __init__(self, reject_after: int = 3, cooldown_steps: int = 5,
                 reactivate_after: int = 3, reliability_low: float = 0.25,
                 reliability_high: float = 0.65, probation_steps: int = 4):
    ...
    def update(self, sensor: str, reliability: float, innovation_consistent: bool,
               packet_fresh: bool = True, alternative_modalities: int = 1) -> RecoveryDecision:
        if not 0.0 <= reliability <= 1.0:
            raise ValueError("reliability must be in [0, 1]")
        status = self._states.setdefault(sensor, _SensorState())
        healthy = reliability >= self.reliability_high and innovation_consistent and packet_fresh
        unhealthy = reliability <= self.reliability_low or not innovation_consistent or not packet_fresh
        ...
        if status.state == RecoveryState.ACTIVE:
            return RecoveryDecision(status.state, 1.0, 1.0, "accept", f"{sensor} is healthy and active")
        if status.state == RecoveryState.DEGRADED:
            scale = max(0.1, reliability)
            return RecoveryDecision(status.state, scale, 1.0 / scale**2, "downweight",
                                    f"{sensor} is degraded; reliability or innovation checks failed")
        if status.state == RecoveryState.COOLDOWN:
            return RecoveryDecision(status.state, 0.0, 100.0, "temporary_reject",
                                    f"{sensor} is in cooldown for {status.cooldown_left} more updates")
        if status.state == RecoveryState.PROBATION:
            scale = min(0.75, (status.probation_step + 1) / max(self.probation_steps, 1))
            return RecoveryDecision(status.state, scale, 1.0 / max(scale, 0.1) ** 2,
                                    "gradual_reactivation",
                                    f"{sensor} passed {status.good_count} consecutive reactivation checks")
        return RecoveryDecision(status.state, 0.0, 100.0, "request_relocalization",
