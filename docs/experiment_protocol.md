...
Estimated trajectories should be evaluated against EuRoC ground truth using timestamp-based matching.

Standalone usage:

```bash
python evaluate_trajectory.py \
  --ground-truth /path/to/mav0/state_groundtruth_estimate0/data.csv \
  --estimated results/trajectories/orbslam3_euroc_mh01.csv \
  --output results/orbslam3_euroc_mh01_eval.json
```
...
This makes it possible to evaluate trajectories generated by ORB-SLAM3, OpenVINS, VINS-Fusion, or later adaptive/self-healing variants using the same interface.
